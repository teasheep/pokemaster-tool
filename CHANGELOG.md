...
兩個管理員同時整理道館拍組名單時會出事 —— 兩個症狀是同一個原因。

### 修正

- **兩位管理員同時把同一隻設為道館拍組，不會再跳出一句資料庫的英文錯誤**
  （`duplicate key value violates unique constraint ...`）。
  後按的那位現在一樣看到「已設為道館拍組」——「這隻在名單裡」本來就已經成立了。
  這個錯誤最麻煩的地方是它會誘導你再按一次，而**那一次是取消，反而真的把它移出名單**。
- **管理員設定道館拍組之後，其他人的「道館拍組」分頁會跟著變**，不必整頁重新載入。
  切回這個分頁時會重新確認一次名單。原本的情況是：名單早就存進去了，
  但別人的畫面停在他剛進來時的那一份 —— 沒有錯誤、沒有載入中，就是少幾張卡，
  完全看不出哪裡不對。
