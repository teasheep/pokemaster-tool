...
- **道館戰看板的即時更新改走 broadcast**（migration `0054`）。`postgres_changes` 的成本是
  O(訂閱人數) —— 每個變更事件，Realtime 伺服器要對**每一個訂閱者各跑一次 RLS**；
  broadcast 只在**訂閱那一刻**授權一次。而且同時連線數是硬牆（超過上限不是變慢，是連不上）。
  同時，broadcast 的 payload 帶著變更後的那一列，client 直接套用 —— 不再「收到事件就重抓整張表」，
  N 個看板的 N 次查詢變成 0 次。順序會變的情況（關卡新增／刪除）與不認得的表才退回重抓。
  資料庫端的 trigger **廣播失敗只 warning 不丟例外**：trigger 丟例外會讓整筆寫入 rollback，
  出刀就報不了。
- **看板不再無條件訂閱**：已結束的賽事不連（它不會再變，卻會一直被人翻），
  分頁切到背景就斷線、回來再連並補一次完整重抓。

### 修正

- **道館戰看板的 realtime 重抓會被人數放大**。看板同時訂了四張表，而一次出刀本來就產生
  **兩個**事件（插 `battle_logs` + `adjust_member_ticket` 更新 `member_tickets`）——
