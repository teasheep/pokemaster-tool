...
- **使用教學**。分兩條路：「我是到館負責人 → 我要建立道館」（4 步）與
  「我是成員 → 我要管理拍組資訊」（6 步）。一般產品導覽那一套：把畫面壓暗、
  只留要講的那個按鈕亮著，旁邊一張卡說明，下一步／上一步／略過。
  **第一次登入落地時自己跳一次**，之後在右上角頭像選單的「使用教學」隨時能再叫出來。
  跨頁的步驟會自己帶你過去（`/gyms?list=1` → 道館的成員頁 → `/pairs` → `/resources`）。
  ...
  按「下一步」不會乾等：**下一頁在你讀這一步的時候就先抓好了**，而框與說明卡的位置
  每一幀直接寫進 DOM（不經 React），所以跟著捲動時是黏著元素走的，不是追在後面。
