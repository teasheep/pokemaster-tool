...
- **道館裡點開拍組，現在跟「我的拍組」是同一個側板了**。原本道館那邊只能看，
  ...
  沒有道理。現在兩邊都是同一顆面板，下拉與左下角都能用。
  **兩邊的欄位完全一樣**：寶數、星數、等級都看得到也改得動（管理員可以直接幫成員設）。
...
- **道館現在看得到、也改得動成員的星數與等級**（點進拍組卡片，跟看自己的一樣要點進去）。
  點卡片左下角調寶數**不會**動到星數與等級 —— 那個手勢管的是寶數，
  不該順手把人家設好的 6★EX 或 Lv200 改掉。
- **修掉一個舊問題**：管理員幫「還沒建立過收藏紀錄」的成員點亮拍組時，
  那筆資料的星數會被記成 5★（不管那隻拍組本來是幾星）。現在記的是正確的原始星級。
- **在道館改別人的寶數，畫面立刻就變**（以前要等伺服器回來才動，下拉會先彈回舊值）。
