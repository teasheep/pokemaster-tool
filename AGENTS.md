...
- **首頁 hero 的插圖是兩塊看板輪替** (`app/home-hero-boards.tsx`, 2026-09-03): 全館拍組持有
  (20 人中幾人有) 與道館戰 (誰還剩幾張券) —— 正好對上標題那句「拍組、道館戰分配」的兩件事, 少一塊
  就有半句沒有畫面。**拍組排第一** (使用者指定, 也是句子的前半)。四件不要改壞的事:
  兩塊**一直都掛著**只是輪流淡入 (grid 疊同一格 + `h-full`), 切換時版面一個像素都不動; 那一下變化 (券 13→12 / 持有 4→5) **由 hero 統一計時** (`ticked` prop) —— 看板自己
  算 timer 的話兩顆都在載入時跑完, 輪到第二塊時動作早就播完了; 點過圓點就停止自動輪替
  (WCAG 2.2.2), `prefers-reduced-motion` 則一開始就不自動輪; 長條的掃入 (`revealed`) **只播第一次**,
  每輪回來都掃一次會變吵。假資料: 成員名一律用主角名
  (小智/小霞/小剛/小光, 一看就知道是範例), 拍組名用**真的存在的拍組** (讀者就是玩這款的人),
  視覺語彙沿用真頁面 (CoverageBar 的門檻與顏色、`remaining/cap` tabular-nums、成員一律走 `MemberAvatar`)。
  **首頁要看得出是哪一款遊戲** (2026-09-03, 使用者:「完全看不出來跟遊戲有關」): 拍組看板每列帶
  「訓練家在前、寶可夢在後」的兩顆疊圓, 道館戰看板的示範成員用他們自己的訓練家立繪當頭貼
  (走 `MemberAvatar` 的 avatarUrl, 不要長第二套頭像實作); 背後再加一層散落的拍組頭像
  `app/home-pair-art.tsx`。這一層也有三件事別改壞:
  1. 氛圍層**中央是挖空的** (radial mask), 所以無論視窗多寬字上面都不會有東西 —— 不要改成整片鋪滿。
  2. 氛圍層**手機整層不渲染** (`hidden sm:block`): 挖空那招靠的是四周有留白, 手機沒有。
     而且那些 `<img>` 一定要 `loading="lazy"` —— **display:none 的 eager 圖 Chrome 照樣會下載**,
     少這一個字手機就白花 12 張圖 (實測 129.5KB → 72.1KB)。
  3. 用到的立繪**只能是已上市拍組** (對過 catalog 的 releaseDate): 這一層是對外的,
     未公布拍組的美術素材不能出現。路徑格式與 sync-pair-card 同一套 `/reference/{trainer,pokemon}/<id>_128.webp`。
