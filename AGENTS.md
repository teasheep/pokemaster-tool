...
- **使用教學是全站唯一的導覽層** (`components/tour/`, 2026-09-06 使用者指定): 兩條路 —
  ...
  **第一次登入落地時自己跳一次**, 之後從**頭像選單**的「使用教學」再叫。
  ...
  16. **教學進行中頂端有一顆狀態膠囊, 而且它的文案分兩種** (2026-09-07 使用者提議加
      「此為教學頁面」的提示, 我改了形式與文案):
      **不是整條橫貫的 bar** —— 整條會蓋住 header, 而教學正需要那排導覽列看得見也點得到
      (指路就是框它)。改成頂端置中的膠囊, 讀起來一樣但不搶版面; 它佔掉的高度會餵給
      `placeCallout` 的 `topInset`, 說明卡才不會被自己的膠囊蓋住。
      **文案只有真的在練習時才說「不會存進資料庫」** —— 其他步驟是真的會寫的
      (例如按下「建立賽事」), 一路寫死等於騙人。
  17. **練習模式綁「那一步該在的那一頁」, 不是只綁步驟** (2026-09-07 修的真 bug):
      只綁步驟的話, 使用者在「側板」那一步跑去 `/resources` 改糖果, 寫入也會被吞掉 ——
      畫面照變、沒有錯誤、什麼徵兆都沒有, 那是很難察覺的資料遺失。
      判斷是 `step.practice && onPage(pathname, resolveAt(step.at))`。
