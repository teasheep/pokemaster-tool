...
- **`/resources` 是「背包」不是「糖果頁」** (2026-09-06 使用者指定「資源」, 2026-09-10 改叫
  ...
  2b. ⚠ **糖果的加減要合併而且要排隊** (2026-09-10 使用者:「為什麼 9/9 草地人會從 5→6?
     是記錄問題還是預設問題?」)。**不是預設問題** —— `member_candies.count` 的 DB 預設是 0,
     沒有任何種子資料。是**寫入順序**的問題: 舊版每按一次就立刻送一個**絕對值** upsert,
     連點時五六個請求同時在飛, 而 **HTTP 不保證到達順序**。兩種症狀都很難懷疑到這裡:
     道館紀錄冒出讀不懂的跳號 (線上 277 筆糖果異動裡有 7 筆: 1→4 / 5→7 / 第一筆就是 3),
     以及**最後存下來的數字少一格** —— 雷歐 2026-09-10 那次畫面顯示 7 而資料庫是 6,
     持續 5 秒; 他當時如果收手就真的少一顆, 而樂觀更新讓畫面永遠是對的, 完全沒有徵兆。
     解法兩層 (`useMyCandies`): `useCoalescedWrite` 讓連點只送最後一個值,
     **再加每一種糖各一條佇列** —— 只有合併沒有佇列的話, 跨視窗的兩趟還是可能後到先至。
...
- **道館紀錄記「成員身上發生的事」+ 道館拍組名單** (pair/candy/ticket/battle_log/gym_pair)。
  ...
  **畫面是兩層** (2026-09-10 使用者:「看不出來是誰改誰的… 不需要一定把所有資訊攤平」):
  ...
  ⚠ **合併不能只比對「上一列」** (2026-09-10 使用者:「紀錄也讓文字合理, 不然使用者覺得奇怪」):
  `mergeRuns` 舊版只跟 `out` 的最後一筆比, 所以**中間夾了別的東西就併不起來**。
  實際發生的: 草地人 44 秒內把五種糖各點了幾下, 排出來是「技術×5, 支援×5, 衝刺,
  支援(5→6), 技術(5→6), 場地, 通用×6」—— 那兩筆單獨的 5→6 跟同一種糖前面那一串不相鄰,
  於是各自留成一列, 畫面上就是一句沒頭沒尾的「技術糖 5 個 → 6 個」(使用者看到的正是這個)。
  改成用 **(成員, 種類, 對象) 當 key** 去找這一組已經開好的那一列, 中間隔了什麼都能併;
  視窗是跟**這一組目前最舊的那一筆**比, 一步一步往回接。
  `mergeRuns` / `isNoop` / `MERGE_WINDOW_MS` 因此搬到 `activity-filters.ts` ——
  那個檔是中立模組 (不可以標 use client), 純函式放那裡才測得到。
  糖果沒有 old_value 時文案要印**「0 個」不是留空**: 留空的話畫面只印一個孤零零的
  「6 個」, 看不出來是從哪裡變過來的。
  `tests/activity-merge.test.ts` 用草地人那 20 筆真實紀錄釘住 (併完要剩 5 列)。
